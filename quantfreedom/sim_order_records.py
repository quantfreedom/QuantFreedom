...
def run_sim_or(
    ...
    logger[LoggerFuncType.Info]("sim_ordder_records.py - run_backtest() - Starting Bar=" + str(starting_bar))
    ...
    logger[LoggerFuncType.Info](
        "sim_ordder_records.py - run_backtest() - account state order results pnl array all set to default"
    )
    ...
    for bar_index in range(starting_bar, total_bars):
        ...
        logger[LoggerFuncType.Info](
            (
                "sim_ordder_records.py - run_backtest() - ind_idx="
                + str(ind_set_index)
                + " dos_idx="
                + str(dos_index)
                + " bar_idx="
        ...
        if order_result.position_size_usd > 0:
            try:
                # checking if sl hit
                logger[LoggerFuncType.Debug]("sim_ordder_records.py - run_backtest() - check_stop_loss_hit")
                ...
                if sl_hit_bool:
                    logger[LoggerFuncType.Debug]("sim_ordder_records.py - run_backtest() - decrease_position")
                    (
                        account_state,
                        order_result,
                    ) = dec_pos_calculator(
                ...
                # checking if liq hit
                logger[LoggerFuncType.Debug]("sim_ordder_records.py - run_backtest() - check_liq_hit")
                ...
                # checking if tp hit
                logger[LoggerFuncType.Debug]("sim_ordder_records.py - run_backtest() - check_tp_hit")
                ...
                logger[LoggerFuncType.Debug]("sim_ordder_records.py - run_backtest() - check_move_stop_loss_to_be")
                ...
                logger[LoggerFuncType.Debug]("sim_ordder_records.py - run_backtest() - check_move_trailing_stop_loss")
            ...
            #     logger[LoggerFuncType.Debug](f"sim_ordder_records.py - run_backtest() - Checking hit Exception -> {e}")
            #     pass
            except Exception:
        ...
        if eval_bool:
            try:
                logger[LoggerFuncType.Debug]("sim_ordder_records.py - run_backtest() - calculate_stop_loss")
                ...
                logger[LoggerFuncType.Debug]("sim_ordder_records.py - run_backtest() - calculate_increase_posotion")
                (
                    average_entry,
                    entry_price,
                    entry_size_asset,
                ...
                logger[LoggerFuncType.Debug]("sim_ordder_records.py - run_backtest() - calculate_leverage")
                (
                    available_balance,
                    cash_borrowed,
                    cash_used,
                ...
                logger[LoggerFuncType.Debug]("sim_ordder_records.py - run_backtest() - calculate_take_profit")
                (
                    can_move_sl_to_be,
                    tp_price,
                    tp_pct,
                ...
                logger[LoggerFuncType.Debug]("sim_ordder_records.py - run_backtest() - Recorded Trade")
                account_state = AccountState(
                    # where we are at
                    ind_set_index=ind_set_index,
                    dos_index=dos_index,
                ...
                logger[LoggerFuncType.Debug]("sim_ordder_records.py - run_backtest() - Account State OrderResult")
            # except Exception as e:
            #     logger[LoggerFuncType.Debug](f"sim_ordder_records.py - run_backtest() - Exception hit in eval strat -> {e}")
            #     pass
            except Exception:
                logger[LoggerFuncType.Debug]("sim_ordder_records.py - run_backtest() - Exception hit in eval strat")
                pass
