...
from quantfreedom.exchanges.apex_exchange.apex_github.eth_signing import SignWithWeb3, SignOnboardingAction
from quantfreedom.exchanges.apex_exchange.apex_github.eth_signing import SignWithKey
...
from quantfreedom.exchanges.apex_exchange.apex_github.starkex.helpers import private_key_to_public_key_pair_hex
...
class HTTP:
    ...
    def __init__(
        ...
        self.logger.info(
            ...
            "from quantfreedom.exchanges.apex_exchange.apex_github import HTTP; "
        )
        ...
        self.network_id = int(network_id or self.network_id or REGISTER_ENVID_MAIN)
        self.env_id = int(env_id or self.env_id or REGISTER_ENVID_MAIN)
    ...
    def configs(self, **kwargs):
        ...
        configs = self._submit_request(method="GET", path=self.endpoint + suffix)
    ...
    def _submit_request(self, method=None, path=None, query=None, headers=None):
        ...
        while True:
            ...
            if method == "GET":
                r = self.client.prepare_request(requests.Request(method, path, params=req_params, headers=headers))
            elif method == "POST":
