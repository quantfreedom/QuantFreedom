...
from quantfreedom.exchanges.binance_exchange.binance_github.websocket.websocket_client import BinanceWebsocketClient
...
class UMFuturesWebsocketClient(BinanceWebsocketClient):
    ...
    def continuous_kline(self, pair: str, contractType: str, interval: str, id=None, action=None, **kwargs):
        ...
        Update Speed: 250ms
        """
        stream_name = "{}_{}@continuousKline_{}".format(pair.lower(), contractType, interval)
    ...
    def diff_book_depth(self, symbol: str, speed=100, id=None, action=None, **kwargs):
        ...
        self.send_message_to_server("{}@depth@{}ms".format(symbol.lower(), speed), action=action, id=id)

    def partial_book_depth(self, symbol: str, level=5, speed=500, id=None, action=None, **kwargs):
        ...
        Update Speed: 250ms, 500ms or 100ms
        """
        self.send_message_to_server("{}@depth{}@{}ms".format(symbol.lower(), level, speed), id=id, action=action)
    ...
    def composite_index(self, symbol: str, id=None, action=None, **kwargs):
        ...
        self.send_message_to_server("{}@compositeIndex".format(symbol.lower()), id=id, action=action)
