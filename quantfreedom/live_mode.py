...
class LiveTrading:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                self.info_logger(f"It took {time_it_took_in_seconds} seconds to download the candles")
                self.info_logger(f"Got a total of {self.exchange.candles_df.shape[0]} candles")
                ...
                if self.strategy.evaluate():
                    ...
                    try:
                        try:
                            ...
                            # place the order
                            self.info_logger("entry_size_asset = round(self.order.entry_size_usd")
                            send_verify_error = False
                            self.info_logger("entry_order_id = self.place_entry_order")
                            entry_order_id = self.place_entry_order(
                                asset_amount=entry_size_asset,
                                entry_price=entry_price,
                            )

                            self.info_logger(f"Submitted entry order -> [order_id={entry_order_id}]")
                            sleep(1)
                            ...
                            self.info_logger(f"checking if in position to cancel tp and sl")
                            ...
                            self.info_logger(f"__set_ex_position_size_asset {self.ex_position_size_asset}")

                            self.info_logger(f"placing stop loss order")
                            sl_order_id = self.place_sl_order(
                                asset_amount=self.ex_position_size_asset,
                                trigger_price=sl_price,
                            )
                            self.info_logger(f"Submitted SL order -> [order_id={sl_order_id}]")

                            sleep(0.5)
                            self.info_logger(f"placing take profit order")
                            tp_order_id = self.place_tp_order(
                                asset_amount=self.ex_position_size_asset,
                                tp_price=tp_price,
                            )
                            self.info_logger(f"Submitted TP order -> [order_id={tp_order_id}]")
                            ...
                            if send_verify_error:
                                ...
                                # )
                                self.info_logger(f"self.email_sender.email_new_order")
    ...
    def place_entry_order(self, asset_amount, entry_price):
        self.info_logger(f"Function: place_entry_order")
    ...
    def send_entry_email(self, entry_price, sl_price, tp_price, liq_price, body):
        ...
        self.info_logger(f"email_sender.email_new_order(body=body,")
    ...
    def __get_pct_dif_below_average_entry(self, price, average_entry):
        self.info_logger(f"__get_pct_dif_below_average_entry(self, price, average_entry")
    ...
    def __create_entry_successful_message(self, entry_order_id, sl_order_id, tp_order_id):
        self.info_logger(f"__create_entry_successful_message(self, entry_order_id, sl_order_id, tp_order_id")
