...
def nb_stdev_tv(
    ...
    """
    avg = -nb_sma_tv(source=source, length=length)
...
def nb_bb_tv(
    ...
    dev = multi * nb_stdev_tv(source=source, length=length)
    upper = basis + dev
    lower = basis - dev
...
def nb_atr_tv(
    ...
    """
    true_range = nb_true_range_tv(candles=candles)
...
def nb_rsi_tv(
    ...
    rma_gains, rma_losses = nb_rma_tv_2(
        source_1=gains,
        source_2=losses,
        length=length,
    )
...
def nb_supertrend_tv(
    ...
    """
    atr = nb_atr_tv(candles=candles, length=atr_length)
...
def nb_squeeze_momentum_lazybear_tv(
    ...
    _, upper_bb, lower_bb = nb_bb_tv(
        source=close,
        length=length_bb,
        multi=multi_bb,
    )

    true_range_ma = nb_sma_tv(
        nb_true_range_tv(candles=candles),
        length=length_kc,
    )

    ma = nb_sma_tv(close, length=length_kc)
