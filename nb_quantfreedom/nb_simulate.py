...
from nb_quantfreedom.nb_helper_funcs import get_dos, get_to_the_upside_nb
from nb_quantfreedom.nb_order_handler.nb_increase_position import AccExOther, OrderInfo
        ...
        for dos_index in range(total_order_settings):
            ...
            for bar_index in range(starting_bar, total_bars):
                ...
                if eval_bool:
                    ...
                    # except Exception as e:
                    #     logger[LoggerFuncType.Debug](
                    #         f"nb_simulate.py - nb_run_backtest() - Exception hit in eval strat -> {e}"
                    #     )
                    #     pass
                    except Exception:
