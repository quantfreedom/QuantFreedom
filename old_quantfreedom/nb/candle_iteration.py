...
class CandleIteration:
    ...
    evaluate_increase_position = (
        None  # signature : (candle, context, configuration, current_positions) -> bool
    )
    evaluate_stop_loss = (
        None  # signature : (candle, context, configuration, current_positions) -> bool
    )
    ...
    def iterate(self):
        ...
        while next_entry_signal_df.empty == False:
            ...
            if self.evaluate_entry_signal(
                current_candle, self.context, self.user_configuration
            ):
                ...
                last_iterated_candle_index = self.__individual_iteration(
                    self.__get_index_from_candle(current_candle) + 1
                )
                next_entry_signal_df = self.__get_next_entry_signal(
                    last_iterated_candle_index
                )
            else:
                next_entry_signal_df = self.__get_next_entry_signal(
                    self.__get_index_from_candle(current_candle)
                )
    ...
    def __get_next_entry_signal(self, current_entry):
        return self.candles[current_entry + 1 :][
            self.entry_signals[current_entry + 1 :]
        ]
    ...
    def __individual_iteration(self, starting_index):
        ...
        for index, candle in iterating_df.iterrows():
            if self.entry_signals[current_index] and self.evaluate_increase_position(
                candle, self.context, self.user_configuration, self.current_positions
            ):
                self.current_positions.append(
                    self.place_entry(self.context, self.user_configuration)
                )  # TODO : can I use the same callback or do I need a different one because it is an increase instead of a first-time entry?
            elif self.evaluate_stop_loss(
                candle, self.context, self.user_configuration, self.current_positions
            ):
            ...
            elif self.evaluate_take_profit(
                candle, self.context, self.user_configuration, self.current_positions
            ):
...
def evaluate_entry_signal(candle, context, configuration):
    logging.info(
        f"[EVALUATING_ENTRY_SIGNAL] [timestamp={str(candle.timestamp)}] [close={candle.close}]"
    )
...
def evaluate_increase_position(candle, context, configuration, current_positions):
    logging.info(
        f"[EVALUATING_INCREASE_POSITION] [timestamp={str(candle.timestamp)}] [close={candle.close}]"
    )
    return False
...
def evaluate_stop_loss(candle, context, configuration, current_positions):
    logging.info(
        f"[EVALUATING_STOP_LOSS] [timestamp={str(candle.timestamp)}] [close={candle.close}]"
    )
...
def evaluate_take_profit(candle, context, configuration, current_positions):
    logging.info(
        f"[EVALUATING_TAKE_PROFIT] [timestamp={str(candle.timestamp)}] [close={candle.close}]"
    )
...
def place_entry(candle, context, configuration):
    logging.info(
        f"[PLACING_ENTRY] [timestamp={str(candle.timestamp)}] [close={candle.close}]"
    )
    return candle
...
def close_position(candle, context, configuration, current_positions):
    logging.info(
        f"[CLOSING_POSITION] [timestamp={str(candle.timestamp)}] [close={candle.close}]"
    )
    return False
...
def adjust_stop_loss_trailing(candle, context, configuration):
    logging.info(
        f"[UPDATING_STOP_LOSS_TRAILING] [timestamp={str(candle.timestamp)}] [close={candle.close}]"
    )
